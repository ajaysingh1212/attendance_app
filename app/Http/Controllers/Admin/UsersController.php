...
class UsersController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = User::with(['roles', 'companies', 'branches'])->select(sprintf('%s.*', (new User)->table));
            ...
            $table->editColumn('degination', function ($row) {
                return $row->degination ? $row->degination : '';
            });
            $table->editColumn('company', function ($row) {
                $labels = [];
                foreach ($row->companies as $company) {
                    $labels[] = sprintf('<span class="label label-info label-many">%s</span>', $company->title);
                }
            ...
            $table->editColumn('branch', function ($row) {
                ...
                foreach ($row->branches as $branch) {
                    $labels[] = sprintf('<span class="label label-info label-many">%s</span>', $branch->title);
                }

                return implode(' ', $labels);
            });
            $table->editColumn('emergency_number', function ($row) {
                return $row->emergency_number ? $row->emergency_number : '';
            });
            ...
            $table->rawColumns(['actions', 'placeholder', 'roles', 'company', 'branch', 'image']);
