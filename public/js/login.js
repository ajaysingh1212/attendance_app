// ---- Rube Goldberg Form — Fixed JS for your HTML ----
document.addEventListener("DOMContentLoaded", () => {
  if (typeof gsap === "undefined") {
    console.error("GSAP missing. Include GSAP before this script.");
    return;
  }
  ...
  // Essential-guard
  const essential = [containerEl, checkboxEl, nameEl, emailEl, submitBtn, gearsContainer];
  if (essential.some(el => !el)) {
    console.error("Some required elements are missing in DOM. Check selectors.");
    return;
  }
  ...
  const state = {
    handClosed: false,
    sumbitBtnOnPlace: false, // (spelling kept)
  ...
  // Keep pulling timeline ref to avoid stacking on change
  let pullingTl = null;
  ...
  window.addEventListener("resize", () => {
    ...
    rAF = requestAnimationFrame(scaleToFit);
  });

  const emailTl = createEmailTl();
  const gearsTls = createGearsTimelines();
  pullingTl = createPullingTimeline(state.handClosed, !!checkboxEl.checked);

  // ------- Events -------
  checkboxEl.addEventListener("change", () => {
    if (pullingTl && pullingTl.kill) pullingTl.kill();
    pullingTl = createPullingTimeline(state.handClosed, checkboxEl.checked);
  });

  nameEl.addEventListener("input", () => {
    nameValid = nameEl.value.trim().length > 3;
    if (nameValid) {
      nameEl.classList.add("valid");
      gearsTls.forEach(tl => {
        if (tl.paused()) {
          tl.play(0);
          gsap.fromTo(tl, { timeScale: 0 }, { timeScale: 1, duration: 0.3 });
        }
      });
    } else {
      ...
      gsap.to(submitBtn, { duration: 0.3, color: "rgba(0,0,0,0)" });
    }
  });
  ...
  submitBtn.addEventListener("click", (e) => {
    e.preventDefault(); // demo: stop real submit; remove if needed
    if (emailValid && checkboxEl.checked && nameValid && sprayRepeatCounter > 1) {
      ...
      gsap.to(".form-row", { delay: 0.4, duration: 0.1, opacity: 0, stagger: 0.1 });
    } else {
      gsap.fromTo(submitBtn, { x: -3 }, { x: 0, duration: 0.2, ease: "power1.inOut", repeat: 2, yoyo: true });
    }
  });
  ...
  function scaleToFit() {
    ...
    if (window.innerHeight < h) {
      ...
      gsap.set(containerEl, { scale: 1, transformOrigin: "50% 50%" });
    }
  }

  function layoutPreparation() {
    if (pullSystemContainer) gsap.set(pullSystemContainer, { x: 375, y: 646 });
    if (sprayHandContainer) gsap.set(sprayHandContainer, { x: 700, y: 621 });
    if (sprayer) gsap.set(sprayer, { x: -59.5, y: 53 });
    if (carContainer) gsap.set(carContainer, { x: 190, y: 802 });
    if (scalesContainer) gsap.set(scalesContainer, { x: 170, y: 710 });
    if (grabbingHand.length) gsap.set(grabbingHand, { x: 297, y: 830 });
    if (grabbingHandClosedFingers.length) gsap.set(grabbingHandClosedFingers, { opacity: 0 });
    if (spiralContainer) gsap.set(spiralContainer, { x: 305, y: 435, svgOrigin: "14 14", scaleX: -1 });
    if (weightBigContainer) gsap.set(weightBigContainer, { x: 305, y: 435 });

    gsap.set(submitBtn, { color: "rgba(0,0,0,0)", transformOrigin: "100% 0%", rotation: -90 });

    if (sprayLines.length || sprayBubbles.length) gsap.set([sprayLines, sprayBubbles], { opacity: 0 });

    if (timingChains[0]) gsap.set(timingChains[0], { attr: { "stroke-width": "5", "stroke-dasharray": "0 12" } });
    if (timingChains[1]) gsap.set(timingChains[1], { attr: { "stroke-width": "5", "stroke-dasharray": "0 12" } });

    if (checkboxPullLine) gsap.set(checkboxPullLine, { attr: { y1: -105, y2: 44 } });
    if (checkboxPullCircle) gsap.set(checkboxPullCircle, { y: 44 });
  }

  function updateSpiralPath(centerX, centerY, radius, coils, points, offset) {
    if (!spiralPath || !weightBig) return;
  ...
  function createEmailTl() {
    ...
    const tl = gsap.timeline({
      ...
      .fromTo(
        ...
        { duration: 0.15, rotation: -1, svgOrigin: "92 20" },
        hammerTimeStart
      )
      ...
      .fromTo(
        ...
        { duration: 0.15, attr: { "stroke-dashoffset": 20 } },
        hammerTimeStart
      )
      .fromTo(
        timingChains[1],
        { attr: { "stroke-dashoffset": 24 } },
        { duration: 0.15, attr: { "stroke-dashoffset": 6 } },
        hammerTimeStart
      )
      ...
      .fromTo(
        car,
        { x: -50 },
        { duration: 0.6, x: 95, ease: "power2.in" },
        hammerTimeStart
      );
  ...
  function createGearsTimelines() {
    ...
    data.forEach((d, dIdx) => {
      ...
      let pathD =
        "M" +
        (gear.center.x + Math.cos(gear.angle - gear.toothAngle + gear.toothCurveAngle) * rOut) +
        ", " +
        (gear.center.y + Math.sin(gear.angle - gear.toothAngle + gear.toothCurveAngle) * rOut) +
        " ";
      ...
      const tl = gsap.timeline({ repeat: -1, paused: true }).to(group, {
      ...
      if (dIdx === data.length - 1) {
        tl.eventCallback("onUpdate", () => {
          ...
          if (pushingHand) gsap.set(pushingHand, { y: deltaY });

          if (deltaY > 8) {
            const d = Math.max(0, deltaY - 8);
            if (sprayerHead) gsap.set(sprayerHead, { y: d });
  ...
  function createPullingTimeline(isFixed, BtnPulled) {
    ...
    function animatePullingLine() {
      if (!btnPullLine || !btnHandlerCircle) return;

      const buttonOriginPoint = [260, -76];
      const btnWidth = 270;

      const deg = ((gsap.getProperty(submitBtn, "rotation") || 0) - 4) * Math.PI / 180;
