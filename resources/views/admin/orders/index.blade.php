            ...
            <table class=" table table-bordered table-striped table-hover datatable datatable-Order">
                <thead>
                    <tr>
                        ...
                        <th>
                            {{ trans('cruds.order.fields.select_product') }}
                        </th>
                        <th>
                            {{ trans('cruds.order.fields.select_customer') }}
                        </th>
                        <th>
                ...
                <tbody>
                    @foreach($orders as $key => $order)
                        <tr data-entry-id="{{ $order->id }}">
                            ...
                            <td>
                                @foreach($order->select_products as $key => $item)
                                    <span class="badge badge-info">{{ $item->name }}</span>
                                @endforeach
                            </td>
                            <td>
                                {{ $order->select_customer->customer_code ?? '' }}
                            </td>
                            <td>
                                {{ $order->select_customer->shop_name ?? '' }}
                            </td>
                            <td>
                                {{ $order->select_customer->owner_name ?? '' }}
                            </td>
                            <td>
                                {{ $order->select_customer->phone_number ?? '' }}
                            </td>
                            <td>
                                {{ $order->select_customer->email ?? '' }}
                            </td>
                            <td>
                                @can('order_show')
