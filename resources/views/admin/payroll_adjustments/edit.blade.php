    ...
    <div class="card-body">
        <form method="POST" action="{{ isset($payrollAdjustment) ? route('admin.payroll-adjustments.update', $payrollAdjustment->id) : route('admin.payroll-adjustments.store') }}">
            ...
            <div class="form-group">
               ...
               <input type="date" name="adjustment_date" value="{{ isset($payrollAdjustment) ? \Carbon\Carbon::parse($payrollAdjustment->adjustment_date)->format('Y-m-d') : now()->toDateString() }}" class="form-control" required>
