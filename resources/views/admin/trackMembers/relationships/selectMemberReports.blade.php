    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-selectMemberReports">
                    <thead>
                        <tr>
                            <th width="10">
                            ...
                            <th>
                                {{ trans('cruds.report.fields.select_member') }}
                            </th>
                            <th>
                    ...
                    <tbody>
                        @foreach($reports as $key => $report)
                            <tr data-entry-id="{{ $report->id }}">
                                ...
                                <td>
                                    {{ $report->select_member->latitude ?? '' }}
                                </td>
                                <td>
                                    {{ $report->select_member->location ?? '' }}
                                </td>
                                <td>
                                    {{ $report->start_date ?? '' }}
                                </td>
...
@endcan
  ...
  let table = $('.datatable-selectMemberReports:not(.ajaxTable)').DataTable({ buttons: dtButtons })
