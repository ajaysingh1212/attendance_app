...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.trackMember.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.track-members.update", [$trackMember->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <h4 class= "text-center mt-2 py-2 ">Edit Track Member</h4>
            <div class="row">
            <div class="form-group col-lg-4">
                ...
                <select class="form-control select2 {{ $errors->has('select_member') ? 'is-invalid' : '' }}" name="select_member_id" id="select_member_id" required>
                    @foreach($select_members as $id => $entry)
                        <option value="{{ $id }}" {{ (old('select_member_id') ? old('select_member_id') : $trackMember->select_member->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-lg-4">
                ...
                <input class="form-control {{ $errors->has('latitude') ? 'is-invalid' : '' }}" type="text" name="latitude" id="latitude" value="{{ old('latitude', $trackMember->latitude) }}" required>
            ...
            <div class="form-group col-lg-4">
                ...
                <input class="form-control {{ $errors->has('longitude') ? 'is-invalid' : '' }}" type="text" name="longitude" id="longitude" value="{{ old('longitude', $trackMember->longitude) }}" required>
            ...
            <div class="form-group col-lg-5">
                ...
                <input class="form-control {{ $errors->has('location') ? 'is-invalid' : '' }}" type="text" name="location" id="location" value="{{ old('location', $trackMember->location) }}" required>
            ...
            <div class="form-group col-lg-5">
                ...
                <input class="form-control {{ $errors->has('time') ? 'is-invalid' : '' }}" type="text" name="time" id="time" value="{{ old('time', $trackMember->time) }}">
                @if($errors->has('time'))
