...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.showReport.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.show-reports.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('select_employess') ? 'is-invalid' : '' }}" name="select_employess_id" id="select_employess_id" required>
                    @foreach($select_employesses as $id => $entry)
                        <option value="{{ $id }}" {{ old('select_employess_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
